 ...
 */
export class StringAdapter implements UpdatableAdapter {
  ...
  public async loadPolicy(model: Model): Promise<void> {
    if (!this.policy) {
      throw new Error('Invalid policy, policy document cannot be false');
    }
  ...
  public updatePolicy(sec: string, ptype: string, oldRule: string[], newRule: string[]): Promise<void> {
    throw new Error('not implemented');
  }
}
