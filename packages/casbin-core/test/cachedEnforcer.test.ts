...
import { Enforcer, newCachedEnforcer, newModelFromString, StringAdapter } from '../src';
...
test('TestRBACModel', async () => {
  const m = newModelFromString(readFileSync('examples/rbac_model.conf').toString());
  const a = new StringAdapter(readFileSync('examples/rbac_policy.csv').toString());
