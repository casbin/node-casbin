...
export class Model {
  ...
  // loadModelFromString loads the model from the string.
  public loadModelFromString(str: string): void {
    const cfg = Config.newConfigFromString(str);
...
/**
 * newModelFromString creates a model from a string.
 */
export function newModelFromString(str: string): Model {
  const m = new Model();
  m.loadModelFromString(str);
  return m;
}
