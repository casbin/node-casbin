...
import { SyncedRoleManager } from '../rbac';
...
export class Assertion {
  ...
  constructor() {
    ...
    this.tokens = [];
    this.policy = [];
    this.rm = new rbac.DefaultSyncedRoleManager(10);
  }

  public async buildIncrementalRoleLinks(rm: rbac.SyncedRoleManager, op: PolicyOp, rules: string[][]): Promise<void> {
    this.rm = rm;
  ...
  public async buildRoleLinks(rm: rbac.SyncedRoleManager): Promise<void> {
    this.rm = rm;
    const count = (this.value.match(/_/g) || []).length;
    if (count < 2) {
      throw new Error('the number of "_" in role definition should be at least 2');
